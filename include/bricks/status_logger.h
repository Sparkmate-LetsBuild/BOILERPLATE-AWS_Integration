...
namespace StatusLogger
{
    ...
    const LOG_NAME_td NAME_ACCEL = "ACCEL";           // Log something to do with the accelerometer
    const LOG_NAME_td NAME_SD = "SD";                 // '' SD
    const LOG_NAME_td NAME_CACHER = "CACHER";         // '' File upload cacher
    const LOG_NAME_td NAME_RTC = "RTC";               // '' RTC
    const LOG_NAME_td NAME_SIMCOM = "SIMCOM";         // '' SIMCOM module (inc. sim card)
    const LOG_NAME_td NAME_UPLOAD = "UPLOAD";         // '' Uploading (inc. InfluxDB && rest POST/GETs)
    const LOG_NAME_td NAME_STRAPI = "STRAPI";         // '' Uploading (inc. InfluxDB && rest POST/GETs)
    const LOG_NAME_td NAME_BUFFER = "BUFFER";         // '' the Upload Buffer
    const LOG_NAME_td NAME_CAN_N2k = "CAN_N2k";       // '' N2k sniffer CAN tranciever
    const LOG_NAME_td NAME_N2k_NOTIF = "N2k_NOTIF";   // '' N2k network itself
    const LOG_NAME_td NAME_ESP32 = "ESP32";           // '' specifically with the ESP32
    const LOG_NAME_td NAME_SUBSAMPLER = "SUBSAMPLER"; // '' the subsampling system
    const LOG_NAME_td NAME_GNSS = "GNSS (SIMCOM)";    // '' the subsampling system
